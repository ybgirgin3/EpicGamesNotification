...
class EpicReminder:
  ...
  def start(self, _):
    ...
    if self.send_mail_button.state:
      ...
      if ret[0]:
        rumps.notification("EpicGamesReminder", "Your Free Games Alert Sent To Your Mail", "")
    if self.save_button_jr.state:
      ret = _save._save(data)
      if ret[0]:
        rumps.notification("EpicGamesReminder", f"Your Free Games File Saved to {ret[1]}", "")
